...
type RestOf<T extends ICustomer | NewCustomer> = Omit<T, 'contractStartDate' | 'contractEndDate'> & {
...
export class CustomerService {
  ...
  protected convertDateFromClient<T extends ICustomer | NewCustomer | PartialUpdateCustomer>(customer: T): RestOf<T> {
    return {
      ...customer,
      contractStartDate: customer.contractStartDate?.format(DATE_FORMAT) ?? null,
      contractEndDate: customer.contractEndDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCustomer: RestCustomer): ICustomer {
    return {
      ...restCustomer,
      contractStartDate: restCustomer.contractStartDate ? dayjs(restCustomer.contractStartDate) : undefined,
      contractEndDate: restCustomer.contractEndDate ? dayjs(restCustomer.contractEndDate) : undefined,
    };
  }
